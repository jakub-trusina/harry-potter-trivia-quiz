...
const assignedHouses = new Map();
const usedHouses = new Set();
export function getHouseFromName(name) {
    ...
    if (isGryffindor && !usedHouses.has('gryffindor'))
        assignedHouse = 'gryffindor';
    else if (isSlytherin && !usedHouses.has('slytherin'))
        assignedHouse = 'slytherin';
    else if (isRavenclaw && !usedHouses.has('ravenclaw'))
        assignedHouse = 'ravenclaw';
    else if (isHufflepuff && !usedHouses.has('hufflepuff'))
        assignedHouse = 'hufflepuff';
    // If no house assigned yet, find the first available house
    if (!assignedHouse) {
        const availableHouses = ['gryffindor', 'slytherin', 'ravenclaw', 'hufflepuff'].filter(house => !usedHouses.has(house));
...
export function resetHouseAssignments() {
...
export function moveModalsToBody() {
...
export function debugDuel(message) {
